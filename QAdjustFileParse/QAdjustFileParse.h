...
namespace In1 {
     ...
     * @brief int文件解析
     * @param stream 文件流
     * @return <QMap{已知点名，点高程}，QMap{<起始点，目标点>，<高差，距离>}>
     */
    QADJUSTFILEPARSE_EXPORT QPair<QMap<QString, double>, QMap<QPair<QString, QString>, QPair<double, double>>> ParseIn1(QTextStream& stream);

    QADJUSTFILEPARSE_EXPORT QPair<QList<In1Known>, QList<In1Observed>> ParseIn12Enity(QTextStream& stream);
}
