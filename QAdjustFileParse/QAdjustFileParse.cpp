...
QPair<QMap<QString, double>, QMap<QPair<QString, QString>, QPair<double, double>>> QAdjustFileParse::In1::ParseIn1(QTextStream& in)
{
    // 是否读到未知点所在行
    bool s = false;
    // 已知点数据
    ...
    return qMakePair(knownPoints, data);
}

QPair<QList<In1Known>, QList<In1Observed>> QAdjustFileParse::In1::ParseIn12Enity(QTextStream& stream)
{
    QList<In1Known> knowns;
    QList<In1Observed> obss;
    auto&& in1 = QAdjustFileParse::In1::ParseIn1(stream);

    auto&& knownDatas = in1.first;
    for (auto&& name : knownDatas.keys()) {
        auto&& altitude = knownDatas[name];
        In1Known in1(name, altitude);
        knowns.append(in1);
    }
    auto&& observedDatas = in1.second;
...
std::tuple<std::tuple<double, double, double, double>, QMap<QString, Eigen::Vector2d>, QList<QPair<QString, QList<std::tuple<QString, QString, double>>>>> QAdjustFileParse::In2::ParseIn2(QTextStream& in)
{
    // 已知误差
    ...
    return std::make_tuple(knownDeviation, knownStns, stnDatas);
}
